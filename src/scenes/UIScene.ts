...
import { GLYPH_TINT, makeSprite } from '../render/drawn';
...
const INK = '#5f5668';
const SCORE_STYLE = { fontFamily: 'sans-serif', fontSize: '30px', color: INK };
const CAPTION_STYLE = {
...
export class UIScene extends Phaser.Scene {
  ...
  /** Last drawn, so text is only re-laid-out when it actually changes. */
  ...
  create(data: { core: Game }): void {
    ...
    this.scoreText = this.add.text(COLUMN_X, SCORE_Y, '0', SCORE_STYLE).setOrigin(0, 0.5);
    this.caption = this.add.text(COLUMN_X, CAPTION_Y, '', CAPTION_STYLE).setOrigin(0, 0);
    ...
    this.lives = Array.from({ length: STARTING_LIVES }, (_unused, life) =>
      makeSprite(this, glyphTextureKey(RED), GLYPH_TINT, 1, {
   ...
   * comparisons rather than a re-layout.
   */
  update(): void {
    ...
    const caption =
      state.tutorialIndex < this.core.tutorial.length
        ? (LEVEL_CAPTIONS[state.tutorialIndex] ?? '')
        : '';
    if (caption !== this.shownCaption) {
      this.shownCaption = caption;
